...
class _GetPathState extends State<GetPath> {
  ...
  /*경로를 나타내기 위해 필요한 변수들*/
  List<Offset> _offsets = [Offset(117, 117)]; //경로를 나타낼 때 필요한 좌표들을 저장하는 리스트
  double nowX = 117;  //사용자의 초기 위치
  double nowY = 117;
    ...
    accelerometerEvents.listen((AccelerometerEvent event) {
      setState(() {
        ...
        if (deltaTimeForDraw >= 2000 && _step > 0) {
          ...
          nowX += _step * 3 * cos(nowYaw * (pi / 180));
          nowY += _step * 3 * sin(nowYaw * (pi / 180));

          //사용자 경로 리스트에 현재 연산한 위치좌표 추가
          _offsets.add(Offset(nowX, nowY));
